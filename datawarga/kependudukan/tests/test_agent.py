...
class MockSystemAdapter(SystemAdapter):
    ...
    def search_kompleks(self, query: str) -> list[dict]:
        ...
        for k in self.kompleks_db:
            if query.lower() in f"{k['blok']}/{k['nomor']}".lower() or query.lower() in k["blok"].lower():
                res.append(k)
        return res
    ...
    def get_kompleks_detail(self, kompleks_id: int) -> dict | None:
        for k in self.kompleks_db:
            if k["id"] == kompleks_id:
                # Mock residents list
                residents = [w for w in self.warga_db if w.get("kompleks_id") == kompleks_id]
    ...
    def add_warga(self, data: dict) -> dict:
        ...
        new_warga = {
            ...
            "alamat": f"Blok ID {data.get('kompleks_id')}" if data.get("kompleks_id") else "Tidak ada alamat"
        }
...
class MockAIProvider(BaseAIProvider):
    ...
    def chat_completion(self, messages: list[dict], response_format: dict = None, correlation_id: str = None) -> str:
        if self.call_count < len(self.responses):
            ...
            self.call_count += 1
            return resp
        return json.dumps({"thought": "default", "tool_call": None, "reply": "Default response"})
...
class AgentTestCase(TestCase):
    def setUp(self):
        ...
        self.adapter.kompleks_db = [
            {"id": 5, "blok": "J2", "nomor": "5", "cluster": "Sektor 1", "rt": "01", "rw": "02"}
        ]
        self.adapter.iuran_db = [
            {"blok_no": "J2/5", "periode_bulan": 1, "periode_bulan_name": "Januari", "periode_tahun": 2026, "total_bayar": 50000, "keterangan": "Lunas"}
        ]
    ...
    def test_out_of_scope_query(self):
        ...
        self.assertIn("hanya dapat membantu Anda dengan tugas-tugas sistem DataWarga", reply)
    ...
    def test_search_warga_tool_flow(self):
        ...
        reply = self.agent.run_agent_turn(conv.messages, self.adapter)
        self.assertEqual(reply, "Saya menemukan warga bernama Budi Santoso (NIK: 1234567890123456) tinggal di Blok J2/5.")
    ...
    def test_add_warga_tool_flow(self):
        ...
        conv = AgentConversation(user_id=123)
        conv.add_user_message("Tolong daftarkan warga baru John Doe, NIK 9876543210123456, Kristen, Laki-laki, lahir di Surabaya 1992-05-15, tinggal di rumah kompleks id 5.")
