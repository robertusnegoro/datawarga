...
async def search_warga(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            "Masukkan kata kunci pencarian. Contoh: /cari John"
        )
        return
    ...
    try:
        ...
        token_response = requests.post(
            ...
            data={"username": settings.BOT_API_USER, "password": settings.BOT_API_PASS},
        )
...
async def search_rumah(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "/" not in search_term:
        await update.message.reply_text(
            "Format alamat salah. Gunakan format: Blok/Nomor (contoh: J2/5)"
        )
        return
...
async def check_iuran(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            "Masukkan alamat rumah. Contoh: /iuran J2/5 2024"
        )
        return
    ...
    try:
        ...
        search_response = requests.post(
            ...
            headers=headers,
            json={"blok_no": search_term, "tahun": year},
        )
        ...
        if not iuran_list:
            await update.message.reply_text(
                f"Belum ada pembayaran iuran untuk {search_term} di tahun {year}."
            )
            return
...
async def bulan_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Masukkan tahun pembayaran:", reply_markup=ReplyKeyboardRemove()
    )
    return TAHUN
...
async def tahun_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(
            "Tahun tidak valid. Masukkan tahun dengan format YYYY (contoh: 2024)"
        )
        return TAHUN
...
async def jumlah_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Upload foto bukti pembayaran:")
    return BUKTI
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Pembayaran dibatalkan.", reply_markup=ReplyKeyboardRemove()
    )
    return ConversationHandler.END
    ...
    try:
        ...
        payment_conv = ConversationHandler(
            ...
            states={
                ...
                KONFIRMASI: [
                    MessageHandler(filters.TEXT & ~filters.COMMAND, konfirmasi_input)
                ],
            },
