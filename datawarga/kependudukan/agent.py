...
SYSTEM_PROMPT = (
    ...
    "   - \"no_kk\" (str, optional)\n"
    "   - \"no_hp\" (str, optional)\n"
    "   - \"status_tinggal\" (str - \"KONTRAK\", \"KOST\", \"TETAP\", \"PINDAH\", \"LAINNYA\", optional)\n"
...
class DjangoSystemAdapter(SystemAdapter):
    def search_warga(self, query: str) -> list[dict]:
        ...
        for w in qs[:10]:
            res.append({
                ...
                "nik": w.nik,
                "no_kk": w.no_kk,
                "alamat": f"Blok {w.kompleks.blok}/{w.kompleks.nomor}" if w.kompleks else "Tidak ada alamat",
    ...
    def get_kompleks_detail(self, kompleks_id: int) -> dict | None:
        ...
        try:
            ...
            residents = Warga.objects.filter(kompleks=k).exclude(status_tinggal="PINDAH")
            res_list = []
            for r in residents:
                res_list.append({
                    "id": r.id,
...
class AgentService:
    ...
    def run_agent_turn(self, messages: list[dict], system_adapter: SystemAdapter, correlation_id: str = None) -> str:
        ...
        for iteration in range(5):
            try:
                ...
                logger.info(f"Agent raw response (iteration {iteration + 1}): {raw_response}")
                ...
                if tool_call and isinstance(tool_call, dict) and tool_call.get("name"):
                    ...
                    logger.info(f"Agent calling tool: {tool_name} with args {args} (iteration {iteration + 1})")
                    ...
                    messages_with_system.append({"role": "assistant", "content": raw_response})
                ...
                logger.error(f"Error in AgentService turn at iteration {iteration + 1}: {str(e)}", exc_info=True)
    ...
    def execute_tool(self, tool_name: str, args: dict, system_adapter: SystemAdapter):
        try:
            if tool_name == "search_warga":
                ...
                return system_adapter.get_kompleks_detail(int(args.get("kompleks_id", 0)))
                ...
                return system_adapter.update_warga(int(args.get("warga_id", 0)), args.get("updates", {}))
            else:
