...
def summarize(year, blok):
    ...
    for rumah in list_kompleks:
        print(rumah)
        iuran = TransaksiIuranBulanan.objects.filter(
            kompleks=rumah.id, periode_tahun=year
        )
...
def create_if_not_exists(year, rumah):
    try:
        get_data = SummaryTransaksiBulanan.objects.get(
            periode_tahun=year, kompleks=rumah
        )
        return get_data
...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--year", required=False, type=int, default=datetime.now().year
        )
