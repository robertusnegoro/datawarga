...
class WargaTestCase(TestCase):
    def setUp(self):
        ...
        self.existing_user = Warga.objects.create(nama_lengkap=self.test_nama_lengkap, nik=self.test_nik)
    ...
    def test_delete_warga_post(self):
        ...
        client = Client()
        client.login(username=self.test_user, password=self.test_pass)
        delete_url = reverse('kependudukan:deleteformWarga', kwargs={'idwarga':warga.id})
